...
## 2025-02-27 - Predictable Temporary Filenames and Insecure Creation
**Vulnerability:** A static temporary filename (`.plans-idx-tmp`) and insecure file creation (`std::fs::File::create`) were used when updating the `plans-index.json`. This predictable name could allow a local attacker to pre-create the file with lenient permissions, or cause a denial of service. The insecure creation defaults to standard permissions, creating a TOCTOU window before atomic rename.
**Learning:** Hardcoded, predictable temporary filenames are susceptible to symlink attacks or pre-creation by other users in shared directories. Creating sensitive files without restricting permissions upfront also poses a TOCTOU risk.
**Prevention:** Use a UUID-based random extension (e.g., `format!("tmp.{}", uuid::Uuid::new_v4())`) to avoid filename collisions. Always use `std::fs::OpenOptions` with `.create_new(true)`, `.write(true)`, `.truncate(true)`, and on Unix `.mode(0o600)` to securely create the temporary file with restricted permissions before writing data.
